...
%>
 <%
  ...
  int student_id=Integer.parseInt(request.getParameter("user_id"));//学生id编号
  int h_id=Integer.parseInt((String)session.getAttribute("h_id"));//得到作业编号
  UserDaoImpl userDao=new UserDaoImpl();
  User student=userDao.find_user(student_id);
  %>
...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
    <legend style="color:red;font-size:25px;">小组成员:<%=student.getU_Name() %>的作业<%=h_id %></legend>
    </fieldset>
    <a href="checkDati.jsp?h_id=<%=h_id %>" class="layui-btn" style="background:red;float:right;">返回</a> 
 ...
 <table class="layui-table">
  ...
  <thead>
<tr>
            <th style="font-size:30px;text-align:center;">题目</th>
			<th style="font-size:30px;text-align:center;">正确答案</th>
			<th style="font-size:30px;text-align:center;">填入答案</th>
			<th style="font-size:30px;text-align:center;">答题结果</th>
...
{ 
	if(operation.getQ_InputResult()==null)
	{
		operation.setQ_InputResult("");
	}
	%>
	<tr>
	<td style="font-size:30px;text-align:center;"><%=operation.getQ_Question() %></td>
	<td style="font-size:30px;text-align:center;"><%=operation.getQ_CorrectResult() %></td>
	<td style="font-size:30px;text-align:center;"><%=operation.getQ_InputResult() %></td>
	<% 
	if(operation.getQ_Status()==0){
	%>
	<td style="font-size:30px;text-align:center;">正确</td>
	<% 
	}else{
	%>
	<td style="font-size:30px;text-align:center;">错误</td>
	</tr>
	<% 
	}
