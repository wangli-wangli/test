...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
    <legend style="color:red;font-size:25px;">三年级上册</legend>
    </fieldset>
...
for(i=0;i<num;)
{
	...
	{
		if(calculateImpl.addTenthousand(add1, add2)!=0)
		{
			shizi[i]=add1+"+"+add2+"=";
			session.setAttribute("Shizi"+i, shizi[i]);
			sum[i]=calculateImpl.addTenthousand(add1, add2);
	...
	{
		//多位数乘一位数
		if(calculateImpl.multiplyMoreOne(multiply1, multiply2)!=0)
		{
			shizi[i]=multiply1+"×"+multiply2+"=";
			session.setAttribute("Shizi"+i,shizi[i]);
			sum[i]=calculateImpl.multiplyMoreOne(multiply1, multiply2);
			session.setAttribute("correct"+i,""+sum[i]);
			%>
			<td><%=shizi[i]%>&nbsp;&nbsp;&nbsp;
            	 <font color=blue ><%=sum[i] %></font>
