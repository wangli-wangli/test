...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
    <legend style="color:red;font-size:25px;">二年级上册</legend>
    </fieldset>
...
for(i=0;i<num;)
{
	...
	int multiply1=random.nextInt(9)+1;
	int multiply2=random.nextInt(9)+1;
	if(random.nextInt(3)==0)
	{
	if(calculateImpl.addHundredAdvance(add1, add2)!=0)
	{
		shizi[i]=add1+"+"+add2+"=";
		session.setAttribute("Shizi"+i, shizi[i]);
		sum[i]=calculateImpl.addHundredAdvance(add1, add2);
...
<%	
	...
	{
		if(calculateImpl.subtractionHundredAdvance(add1,add2)!=0)
		{
			shizi[i]=add1+"－"+add2+"=";
			session.setAttribute("Shizi"+i, shizi[i]);
			sum[i]=calculateImpl.subtractionHundredAdvance(add1, add2);
			session.setAttribute("correct"+i,""+sum[i]);
			%>
			<tr align="center">
			 <td><%=shizi[i]%>&nbsp;&nbsp;&nbsp;
	...
	{
		if(calculateImpl.multiplyNine(multiply1, multiply2)!=0)
		{
			shizi[i]=multiply1+"×"+multiply2+"=";
			session.setAttribute("Shizi"+i, shizi[i]);
			sum[i]=calculateImpl.multiplyNine(multiply1, multiply2);
			session.setAttribute("correct"+i,""+sum[i]);
			%>
			<tr align="center">
			 <td><%=shizi[i]%>&nbsp;&nbsp;&nbsp;
