...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
    <legend style="color:red;font-size:25px;">三年级下册</legend>
    </fieldset>
...
for(i=0;i<num;)
{
	...
	int multiply2=random.nextInt(100)+1;
	int dividend=random.nextInt(100)+1;
	int divid=random.nextInt(9)+1;
	
	if(random.nextInt(2)==0)
	{
		//两位数相乘
		if(calculateImpl.multiplyTwo(multiply1, multiply2)!=0)
		{
			shizi[i]=multiply1+"×"+multiply2+"=";
			session.setAttribute("Shizi"+i, shizi[i]);
			sum[i]=calculateImpl.multiplyTwo(multiply1, multiply2);
	...
	{
		//多位数除一位数
		if(calculateImpl.dividMoreOne(dividend, divid)!=0)
		{
			shizi[i]=dividend+"÷"+divid+"=";
			session.setAttribute("Shizi"+i,shizi[i]);
			sum[i]=calculateImpl.dividMoreOne(dividend, divid);
			session.setAttribute("correct"+i,""+sum[i]);
			%>
			<td><%=shizi[i]%>&nbsp;&nbsp;&nbsp;
            	 <font color=blue ><%=sum[i] %></font>
