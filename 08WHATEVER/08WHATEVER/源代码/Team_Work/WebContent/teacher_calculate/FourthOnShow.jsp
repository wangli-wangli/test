...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
    <legend style="color:red;font-size:25px;">四年级上册</legend>
    </fieldset>
...
for(i=0;i<num;)
{
	int multiply1=random.nextInt(1000)+1;
	int multiply2=random.nextInt(1000)+1;
	
	int dividend=random.nextInt(1000)+1;
	int divid=random.nextInt(100)+1;
	
	if(random.nextInt(2)==0)
	{
		//三*二
		if(calculateImpl.multiplyThreeTwo(multiply1, multiply2)!=0)
		{
			shizi[i]=multiply1+"×"+multiply2+"=";
			session.setAttribute("Shizi"+i, shizi[i]);
			sum[i]=calculateImpl.multiplyThreeTwo(multiply1, multiply2);
	...
	{
		//除数是两位数
		if(calculateImpl.dividTwo(dividend, divid)!=0)
		{
			shizi[i]=dividend+"÷"+divid+"=";
			session.setAttribute("Shizi"+i,shizi[i]);
			sum[i]=calculateImpl.dividTwo(dividend, divid);
			session.setAttribute("correct"+i,""+sum[i]);
			%>
